...
export default class SCLSettingTab extends PluginSettingTab {
	...
	override getControlValue(key: string): unknown {
		...
		if (key === "enableTagChips") return this.plugin.settings.enableTagChips;
		if (key === "getFromInlineField") return this.plugin.settings.getFromInlineField;
	...
	private handleGlobalSettingUpdate(key: string, value: unknown): void {
		const settingsProxy = this.plugin.settings as unknown as Record<string, unknown>;
		...
		for (let i: number = 0; i < selectorsCount; i++) {
			...
			if (noteEl !== null) {
				...
				if (rule.type === "tag") {
					...
					for (let k = 0; k < boldCount; k++) {
						const bEl = boldElements[k];
						if (bEl instanceof HTMLElement) {
							if (k === 0) bEl.setText(attrName);
							if (k === 1) bEl.setText(valText);
						}
					}
				} else {
				...
				const oldIconsCount = oldIcons.length;
				for (let j = 0; j < oldIconsCount; j++) {
					const icon = oldIcons[j] ?? null;
