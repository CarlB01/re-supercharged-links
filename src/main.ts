...
export default class ResuperchargedLinks extends Plugin {
    ...
    async onload(): Promise<void> {
        console.log('Re-Supercharged Links: Initializing plugin engine...');
            ...
            plugin.observers.forEach(([_, type, ownClass]) => {
                ...
                leaves.forEach(leaf => {
                    if (leaf?.view?.containerEl) {
                        plugin.updateContainer(leaf.view.containerEl, plugin, ownClass);
                    }
                });
            });
        }, 500, true);
    ...
    updateContainer(container: HTMLElement, plugin: ResuperchargedLinks, selector: string, filterCollapsible = false): void {
        ...
        nodes.forEach(node => {
            if (node instanceof HTMLElement) {
                updateDivExtraAttributes(plugin.app, plugin, node, "", undefined, filterCollapsible);
            }
        });
    }
